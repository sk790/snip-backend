...
app.put(
  ...
  protect,
  async (req: AuthRequest, res: Response): Promise<any> => {
    const { originalUrl, startDate, expiresAt, password, customUrlCode, isQr } = req.body;
    ...
    try {
      ...
      if (customUrlCode && customUrlCode !== url.urlCode) {
        const existing = await Url.findOne({
          urlCode: customUrlCode,
          isDeleted: false,
        });
        ...
        url.urlCode = customUrlCode;
        url.shortUrl = `${process.env.BASE_URL}/${customUrlCode}`;
        url.isCustom = true;
      }

      if (startDate !== undefined) url.startDate = startDate ? new Date(startDate) : undefined;
      ...
      if (isQr !== undefined) url.isQr = !!isQr;
